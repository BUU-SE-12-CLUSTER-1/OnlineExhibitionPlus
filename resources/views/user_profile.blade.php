...
<img src="" alt="">
<form action={{url("/update-user-detail/".$oe_users['user_id']."/fname")}} method="POST">
    @csrf
    <p>Name :
...
?>
<form action={{url("/update-user-detail/".$oe_users['user_id']."/major")}} method="POST">
    @csrf
    <p>Major :
...
<button id="save-major" style="display: none" type="submit">save</button>
    <button id="cancel-major" onclick="cancel('major')" style="display: none" type="button" >cancel</button>
</p>
</form>
<h2>Contact</h2>
<p>------------------------------------------------------------</p>
<form action={{url("/update-user-detail/".$oe_users['user_id']."/email")}} method="POST">
    @csrf
    <p>Email :
    <label id="txt-user-email">{{$oe_users['user_email']}}</label>
    <input type="text" value="{{$oe_users['user_email']}}" id="input-user-email" name="user_email" style="display: none">
    <button id="edit-email" onclick="edit('email')" type="button">edit</button>
    <button id="save-email" style="display: none" type="submit">save</button>
    <button id="cancel-email" onclick="cancel('email')" style="display: none" type="button" >cancel</button>
</p>
</form>
<form action={{url("/update-user-detail/".$oe_users['user_id']."/phone")}} method="POST">
    @csrf
    <p>Phone number :
    <label id="txt-user-phone">{{$oe_users['user_phone']}}</label>
    <input type="text" value="{{$oe_users['user_phone']}}" id="input-user-phone" name="user_phone" style="display: none">
    <button id="edit-phone" onclick="edit('phone')" type="button">edit</button>
    <button id="save-phone" style="display: none" type="submit">save</button>
